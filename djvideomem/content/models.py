...
class Subscription(models.Model):
    ...
    def __str__(self): 
        return self.user.email # pylint: disable=maybe-no-member
...
def post_email_confirmed(request, email_address, *args, **kwargs):
    ...
    free_trial_pricing = Pricing.objects.get(name='Free Trial') # pylint: disable=maybe-no-member
    subscription = Subscription.objects.create( # pylint: disable=maybe-no-member
        user=user, 
    ...
    stripe_subscription = stripe.Subscription.create(
        ...
        items=[{'price': 'price_1HQrOGA85dpmuOhtZYgtggLx'}],
        trial_period_days = 3
    )
